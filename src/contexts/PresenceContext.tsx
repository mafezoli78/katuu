...
import type { Intention, Presence, NearbyTemporaryPlace } from '@/hooks/presence/types';

// Sub-hooks
import { usePresenceTimer } from '@/hooks/presence/usePresenceTimer';
import { usePresenceGPS } from '@/hooks/presence/usePresenceGPS';
import { usePresenceState } from '@/hooks/presence/usePresenceState';
import { usePresenceLifecycle } from '@/hooks/presence/usePresenceLifecycle';

/**
 * FONTE ÚNICA da presença do usuário.
 *
 * Antes, `usePresence` era um hook: cada tela que o chamava (Home, Location,
 * Profile/PresenceStatusCard) criava sua PRÓPRIA instância — com seu próprio
 * currentPresence, timer, watcher de GPS e listeners de visibilidade. Telas
 * dessincronizavam (ex.: card de presença não sumia do Perfil após expirar,
 * porque o timer que disparou o fim estava noutra instância).
 *
 * Agora a lógica roda UMA vez, aqui no Provider. `usePresence()` só lê deste
 * contexto. A sincronia entre telas é por construção: existe um estado só.
 *
 * A lógica abaixo é a mesma de antes (movida do antigo hook), sem mudança de
 * comportamento — apenas deixou de ser multiplicada.
 */
function usePresenceController() {
  const { user } = useAuth();
...
type PresenceContextValue = ReturnType<typeof usePresenceController>;
...
export function usePresence(): PresenceContextValue {
  ...
  if (!ctx) {
    throw new Error('usePresence precisa estar dentro de <PresenceProvider>');
  }
  return ctx;
}
