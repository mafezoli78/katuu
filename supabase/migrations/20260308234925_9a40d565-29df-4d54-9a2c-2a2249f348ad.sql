...
-- Create updated function with match_score and new ordering logic
CREATE OR REPLACE FUNCTION public.get_users_at_place_feed(p_user_id uuid, p_place_id uuid)
 RETURNS TABLE(
    ...
    presence_inicio timestamp with time zone,
    match_score integer
)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Fetch current user's presence start time
  SELECT inicio INTO v_my_presence_inicio
...
CREATE INDEX IF NOT EXISTS idx_presence_place_inicio ON public.presence(place_id, inicio DESC);
