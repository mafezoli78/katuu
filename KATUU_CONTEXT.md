...
- **CENTRALIZAR ESTADO — PRÓXIMO ÉPICO PRIORITÁRIO (decisão Fabricio 17/06).** Princípio firmado: **uma fonte de verdade por coisa; todo mundo busca de lá em vez de refazer.** Hoje o estado está espalhado e cada tela cuida do seu — é a causa-raiz de uma CLASSE inteira de bugs de estado dessincronizado (card de presença não some do Perfil após expirar; "aceno aceito" não vira "chat em andamento"; e a confusão de "duas leituras de GPS divergentes" na investigação da listagem). Dois alvos concretos:
  ...
  - **PRINCÍPIO DE ARQUITETURA (firmado Fabricio 17/06, refinado):** "se muitos arquivos precisam de uma informação/regra, essa regra vira FONTE DE VERDADE única — de preferência no BACKEND." "Arquivo" é metáfora: o que importa é UM lugar canônico que define a regra; todos consultam, ninguém repete. **Backend porque a regra é GARANTIA, não preferência de UI** (regra 10: cliente hackeado não pode quebrá-la). No banco a forma canônica é **view e/ou função-helper** (ex.: uma view `active_presences` encapsula "o que conta como presente"; funções consultam a view em vez de repetir o `WHERE`). O cliente pode ESPELHAR a regra para UX (mostrar a tela certa), mas a verdade é server-side; se discordarem, o backend vence (mesmo padrão do `confirm_presence` ≤300m).
  - **1ª APLICAÇÃO — UNIFICAR PRESENÇA com `entry_type` (decisão Fabricio 17/06):** dois tipos de usuário no local — **presente** (`entry_type='presence'`) e **explorador** (`entry_type='exploration'`). Hoje o explorador NÃO tem registro (invisibilidade "física"). Decisão: ele passa a TER linha em `presence` com a flag, e a invisibilidade vira LÓGICA (derivada da flag). Ganhos: resolve o bug da selfie de graça (ver §Explorador/12.4 — a policy de Storage "view selfies from same location" passa a valer pro explorador porque ele está no local), unifica o modelo (pode convergir `get_place_explore_feed` e `get_users_at_place_feed`), e dá relatório melhor ("20 presentes, 15 explorando"). RISCO: invisibilidade lógica é mais frágil que física — se UMA query esquecer de excluir explorador, ele VAZA pro feed real. Por isso a fonte de verdade (view) é essencial: centraliza o filtro num lugar.
  - **LEVANTAMENTO (briefing da sessão de implementação) — 16 funções no banco tocam `presence`:**
    - EXCLUIR explorador: `get_users_at_place_feed` (feed real), contagem `active_users` (`find_nearby_temporary_places` l.1098), `send_wave` (co-presença), `get_active_conversations` (co-presença), `can_auto_end_presence`, `confirm_presence`.
    - INCLUIR/tratar: `get_place_explore_feed` (é o feed dele), `activate_presence` (gravar `entry_type`; entrada de explorador é mais enxuta — sem selfie/intention/assunto, talvez função própria `enter_as_explorer`).
    - DECISÕES DE REGRA AINDA ABERTAS (responder na sessão): ciclo de vida do explorador (Fabricio quis "decidir na implementação" — candidato: expira rápido/ao sair da tela, regra própria curta, NÃO 2h); explorador tem card próprio? (`get_my_presence_card`/`update_my_presence_card` — provável que não); pode denunciar explorando? (`submit_report` — provável que sim); `end_presence_cascade` ao sair da exploração (provável inócuo, explorador não tem waves/conversas).
    - CLIENTE espelha: `usePresence`, `usePeopleNearby`, contagem, sweeper, e a tela do Explore.
  - **COMO FAZER:** sessão própria dedicada, Opus + plan, igual à reescrita da listagem. NÃO tocar as 16 funções no improviso (cada filtro esquecido = vazamento de explorador; é o bug do `.limit` multiplicado por 16). Implementar a fonte de verdade (view/helper) PRIMEIRO, depois migrar as funções para consultá-la.
